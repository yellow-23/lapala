...
```
frontend/       Astro + React + Tailwind (frontend, standalone)
  src/features/
    jobs/       listado, filtros, cards
    cv/         builder, preview (Fase 2)
    ai/         generador, match (Fase 3)
backend/        FastAPI — /cv/render, /ai/generate-cv, /ai/match
scrapers/       Python: ChiletrabajosSource, ComputrabajoSource, BNESource, GetOnBoardSource
...
- **Infra:** GitHub Actions (ingesta), Cloudflare Pages (frontend — lapala.pages.dev), Render free (backend — lapala.onrender.com)
...
- **Supabase client lazy** en el frontend: usar `getSupabase()` de `frontend/src/lib/supabase.ts`, nunca instanciar en el módulo (rompe build estático de Astro).
- **RLS activo.** `jobs` lectura pública. `cvs`, `profiles`, `matches` solo el dueño via `auth.uid()`.

## Variables de entorno

```
# frontend/.env (expuestas al browser, prefijo PUBLIC_)
PUBLIC_SUPABASE_URL=
PUBLIC_SUPABASE_ANON_KEY=

# backend/.env y scrapers/.env (solo servidor)
SUPABASE_URL=
